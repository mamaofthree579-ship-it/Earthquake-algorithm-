...
st.title("Enhanced Stochastic Risk Model")
...
def fetch_eq():
    r = requests.get(
        ...
        params={"format":"geojson",
                "starttime":(date.today()-timedelta(days=7)).isoformat(),
...
# station lookup stub (use 9447130 for demo)
...
tide_vals = [float(x["v"]) for x in tide.get("predictions",[])][:len(df)]
...
for _ in range(ens):
    ...
    for n in range(len(df)):
        ...
        G = math.cos(0.008*n)
        V = event.mag
        tide_part = tide_vals[n] if n < len(tide_vals) else 0.0
        O = chi * (tide_part + enso_val)
    ...
    P = 1/(1+np.exp(-I)).clip(0.01,0.99)
    Ps.append(P)

df["P_mean"] = np.mean(Ps, axis=0)
df["P_std"] = np.std(Ps, axis=0)
df["Risk"] = df["P_mean"].apply(lambda p: "Low" if p<0.25 else "Moderate" if p<0.5 else "Elevated" if p<0.75 else "Critical")

st.subheader("Recent risk (ensemble)")
st.dataframe(df[["date","place","mag","P_mean","P_std","Risk"]])
